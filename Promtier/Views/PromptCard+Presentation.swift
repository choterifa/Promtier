...
extension PromptCard {
    ...
    // MARK: - Layout Helpers (Fase 11 Optimization)

    var glowFillColor: Color {
        if isRecommended {
            return themeColor.opacity(isGlowAnimating ? 0.15 : 0.05)
        } else if isSelected || effectiveHover {
            return themeColor.opacity(0.08)
        }
        return Color.clear
    }

    var glowBlurRadius: CGFloat {
        if isRecommended {
            return isGlowAnimating ? 15 : 8
        } else if preferences.isHaloEffectEnabled && effectiveHover {
            return 12
        }
        return 0
    }

    var glowStrokeColor: Color {
        if isRecommended {
            return themeColor.opacity(isGlowAnimating ? 0.8 : 0.3)
        }
        return cardBorderColor
    }

    var glowStrokeLineWidth: CGFloat {
        if isRecommended {
            return 1.5
        }
    ...
    var cardShadowColor: Color {
        if isRecommended {
            return themeColor.opacity(isGlowAnimating ? 0.4 : 0.1)
        }
        return .black.opacity(effectiveHover ? 0.05 : (isPerformanceMode ? 0.01 : 0.0))
    }
